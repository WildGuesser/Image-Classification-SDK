...
(async () => {
  ...
  for (const file of hemlockFiles) {
    const imageData = fs.readFileSync(`${hemlockDir}/${file}`);
    fileUploadPromises.push(
      trainer.createImagesFromData(sampleProject.id, imageData, {
        tagIds: [hemlockTag.id],
      })
    );
    await sleep(1000); // Sleep for 2 seconds after adding each image
  }

  const cherryDir = `${sampleDataRoot}/Japanese_Cherry`;
  const japaneseCherryFiles = fs.readdirSync(cherryDir);

  for (const file of japaneseCherryFiles) {
    const imageData = fs.readFileSync(`${cherryDir}/${file}`);
    fileUploadPromises.push(
      trainer.createImagesFromData(sampleProject.id, imageData, {
        tagIds: [cherryTag.id],
      })
